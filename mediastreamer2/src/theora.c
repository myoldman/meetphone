...
static void enc_init(MSFilter *f){
	...
	s->tinfo.keyframe_data_target_bitrate=(ogg_uint32_t)(s->tinfo.target_bitrate*1.2);
...
static int enc_set_fps(MSFilter *f, void *data){
	...
	EncState *s=(EncState*)f->data;
	s->tinfo.fps_numerator=(ogg_uint32_t)*fps;
	s->tinfo.keyframe_frequency=(ogg_uint32_t)((*fps)*5);
	s->tinfo.keyframe_frequency_force=(ogg_uint32_t)((*fps)*5);
	return 0;
}
...
static int enc_get_fps(MSFilter *f, void *data){
	EncState *s=(EncState*)f->data;
	float *fps=(float*)data;
	*fps=(float)s->tinfo.fps_numerator;
	return 0;
}
...
static int enc_set_br(MSFilter *f, void*data){
	...
	fps=(float)s->tinfo.fps_numerator;
	s->tinfo.target_bitrate=(int)(codecbr*0.9);
	s->tinfo.keyframe_data_target_bitrate=(ogg_uint32_t)codecbr;
