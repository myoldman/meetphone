...
static HWND create_window(int w, int h)
{
    ...
    //SetWindowLong(hwnd, GWL_EXSTYLE, currentWindowLong^0x80000);
	//SetLayeredWindowAttributes(hwnd, 0, 255 , 3);
	return hwnd;
}
...
static void dd_display_process(MSFilter *f){
	...
	if (f->inputs[0]!=NULL && (main_im=ms_queue_peek_last(f->inputs[0]))!=NULL) {
		if (ms_yuv_buf_init_from_mblk(&mainpic,main_im)==0){
			if (obj->autofit && (obj->vsize.width!=mainpic.w || obj->vsize.height!=mainpic.h)){
				...
				POINT ptDiff;
				ms_message("Detected video resolution changed, resizing window %d %d", mainpic.w, mainpic.h);
				GetWindowRect(obj->window,&cur);
				wsize.width=mainpic.w;
				wsize.height=mainpic.h;
				GetClientRect(obj->window, &rcClient);
				GetWindowRect(obj->window, &rcWindow);
				ptDiff.x = (rcWindow.right - rcWindow.left) - rcClient.right;
				ptDiff.y = (rcWindow.bottom - rcWindow.top) - rcClient.bottom;

				MoveWindow(obj->window,cur.left, cur.top, wsize.width+ptDiff.x, wsize.height+ptDiff.y,TRUE);
	...
	if (main_im!=NULL || local_im!=NULL || obj->need_repaint){
		PostMessage(obj->window, WM_MEMBER_STOP_LOADING, 0, 0);
		...
		if (obj->locview.rgb!=NULL){
			...
			tmp_bmp=CreateCompatibleBitmap(hdc,wsize.width - 1, wsize.height - 1);
		...
		if(hdc2 != NULL && obj->window_title != '\0')
		{
			...
			SelectObject(hdc2,hf); 
			SetBkMode(hdc2,TRANSPARENT); 
			SetTextColor(hdc2,RGB(255,255,255));
			DrawText(hdc2,obj->window_title,-1,&rc,DT_LEFT);       //添加字符上去
