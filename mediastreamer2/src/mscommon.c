...
int ms_load_plugins(const char *dir){
	...
	*p = '\0';
	...
	snprintf(szDirPath, sizeof(szDirPath), "%s%s\\*.dll", szBuf, dir);
...
extern MSWebCamDesc rtsp_card_desc;//added by liuhong for rtsp preview
...
static MSWebCamDesc * ms_web_cam_descs[]={
...
#endif
#if defined(__MINGW32__) || defined (HAVE_DIRECTSHOW)
	//&ms_push_source_cam_desc,//added by liuhong for desktop push
#endif
	...
	&mire_desc,
	&static_image_desc,
	&rtsp_card_desc,//added by liuhong for rtsp
#endif
...
unsigned long ms_concealer_context_get_sampling_time(MSConcealerContext* obj) {
	return (unsigned long)obj->sample_time;
}
...
unsigned int ms_concealer_context_is_concealement_required(MSConcealerContext* obj,uint64_t current_time) {
	...
	if (obj->sample_time < current_time && (unsigned int)obj->plc_count<obj->max_plc_count) {
		obj->plc_count++;
		obj->total_number_for_plc++;
	} else {
		obj->plc_count=0;
		if ((unsigned int)obj->plc_count>=obj->max_plc_count) {
			/*reset sample time*/
			obj->sample_time=0;
		}
	}
