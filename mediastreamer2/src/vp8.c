...
static void enc_init(MSFilter *f) {
	...
	s->cfg.rc_target_bitrate = (unsigned int)(((float)s->bitrate)*0.92/1024.0); //0.9=take into account IP/UDP/RTP overhead, in average.
	s->cfg.g_pass = VPX_RC_ONE_PASS; /* -p 1 */
	s->fps=15;
	s->cfg.g_timebase.num = 1;
	s->cfg.g_timebase.den = (int)s->fps;
...
static void vp8_fragment_and_send(MSFilter *f,EncState *s,mblk_t *frame, uint32_t timestamp, const vpx_codec_cx_pkt_t *pkt, bool_t lastPartition){
	...
	for (rptr=frame->b_rptr;rptr<frame->b_wptr;){
		...
		len=MIN(s->mtu,(unsigned int)(frame->b_wptr-rptr));
		packet=dupb(frame);
		packet->b_rptr=rptr;
