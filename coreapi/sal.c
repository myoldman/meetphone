...
void sal_op_set_userid(SalOp *op, const char *userid){
	assign_string(&((SalOpBase*)op)->userid,userid);
}
...
const char *sal_op_get_username(const SalOp *op){
	return ((SalOpBase*)op)->username;
}
