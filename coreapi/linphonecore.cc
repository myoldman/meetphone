...
static void call_logs_read_from_config_file(LinphoneCore *lc){
	...
	for(i=0;;++i){
		...
		if (lp_config_has_section(cfg,logsection)){
			LinphoneCallLog *cl=ms_new0(LinphoneCallLog,1);
			cl->dir=(LinphoneCallDir)lp_config_get_int(cfg,logsection,"dir",0);
			cl->status=(LinphoneCallStatus)lp_config_get_int(cfg,logsection,"status",0);
...
static void net_config_read (LinphoneCore *lc)
{
	...
	linphone_core_set_firewall_policy(lc,(LinphoneFirewallPolicy)tmp);
...
static void sip_config_read(LinphoneCore *lc)
{
	...
	if (lp_config_get_int(lc->config,"sip","sip_random_port",0)) {
		...
		tr.udp_port=lp_config_get_int(lc->config,"sip","sip_port",35060);
	}
...
static void rtp_config_read(LinphoneCore *lc)
{
	...
	port=lp_config_get_int(lc->config,"rtp","rtsp_audio_rtp_port",17078);
	linphone_core_set_audio_rtsp_port(lc,port);

	port=lp_config_get_int(lc->config,"rtp","rtsp_video_rtp_port",19078);
	ms_message("port is %d", port);
	linphone_core_set_video_rtsp_port(lc,port);
...
static void codecs_config_read(LinphoneCore *lc)
{
	...
	for (i=0;get_codec(lc->config,"video_codec",i,&pt);i++){
		if (pt){
			if (!ms_filter_codec_supported(pt->mime_type)){
				ms_warning("Codec %s is not supported by mediastreamer2, removed.",pt->mime_type);
			}else video_codecs=codec_append_if_new(video_codecs,(PayloadType *)pt);
		}
	}
...
static void video_config_read(LinphoneCore *lc){
	...
	lc->video_conf.rtsp_preview_url = ms_strdup(lp_config_get_string(lc->config,"video","rtsp_preview_url",NULL));
	...
	for (i=0;elem!=NULL;elem=elem->next,i++){
		...
		if(strcasecmp(cam->desc->driver_type, "RTSP") == 0) {
			if(lc->video_conf.rtsp_preview_url != NULL) {
				cam->name=ms_strdup(lc->video_conf.rtsp_preview_url);
			} else {
				cam->name=ms_strdup("RTSP Camera");
			}
			ms_free(cam->id);
			cam->id=ms_strdup_printf("%s: %s",cam->desc->driver_type,cam->name);
		}
    ...
    const char *factory_config_path, void * userdata)
{
	...
	lc->is_admin = FALSE;// added by liuhong for noc
...
// added by liuhong for rtsp call start
 ...
 * @ingroup network_parameters
**/
int linphone_core_get_audio_rtsp_port(const LinphoneCore *lc)
{
	return lc->rtp_conf.audio_rtsp_rtp_port;
}
 ...
 * @ingroup network_parameters
**/
int linphone_core_get_video_rtsp_port(const LinphoneCore *lc){
	return lc->rtp_conf.video_rtsp_rtp_port;
}


/**
 * Sets the UDP port used for rtsp audio streaming.
 *
 * @ingroup network_parameters
**/
void linphone_core_set_audio_rtsp_port(LinphoneCore *lc, int port)
{
	lc->rtp_conf.audio_rtsp_rtp_port=port;
}

/**
 * Sets the UDP port used for rtsp video streaming.
 *
 * @ingroup network_parameters
**/
void linphone_core_set_video_rtsp_port(LinphoneCore *lc, int port){
	lc->rtp_conf.video_rtsp_rtp_port=port;
}
 ...
 * @param lc The LinphoneCore object
**/
const char * linphone_core_get_rtsp_preview_url(LinphoneCore *lc)
{
	return lc->video_conf.rtsp_preview_url;
}

// added by liuhong for rtsp call end
...
void linphone_core_iterate(LinphoneCore *lc){
	...
	while(calls!= NULL){
		...
		if (call->dir==LinphoneCallIncoming && call->state==LinphoneCallOutgoingRinging){
			elapsed=(int)(curtime-call->start_time);
...
LinphoneCall *linphone_core_invite_desktop_share(LinphoneCore *lc, const char *url) {
	LinphoneCall *call;
	LinphoneCallParams *p=linphone_core_create_default_call_parameters (lc);
...
void linphone_core_rtsp_teardown_for_call(LinphoneCall *call){
	ms_rtsp_disconnect(call->remoteRtspClient, call->remoteRtspMediaSession);
	call->remoteRtspMediaSession = NULL;
	call->remoteRtspClient = NULL;
}

LinphoneCall * linphone_core_rtsp_desc(LinphoneCore *lc, const char *url, const char *name){
	char dest_addr[64] = {0};
	int port_offset;
	char to_uri[64] = {0};
	LinphoneCallParams *p=linphone_core_create_default_call_parameters (lc);
	LinphoneCall *call=ms_new0(LinphoneCall,1);
	if(lc->current_rtsp_call != NULL){
		linphone_core_terminate_call(lc, lc->current_rtsp_call);
	}
	...
	linphone_core_get_local_ip(lc, url ,call->localip);
	...
	call->audio_port=linphone_core_get_audio_rtsp_port(call->core)+port_offset;
	call->video_port=linphone_core_get_video_rtsp_port(call->core)+port_offset;
	
	call->params=*p;
	
	call->localdesc=create_local_media_description (lc,call);
	call->camera_active=p->has_video;
	...
	if (lc->vtable.display_status!=NULL) {
		char *barmsg = ortp_strdup_printf("%s %s", _("Contacting"), name);
		lc->vtable.display_status(lc, barmsg);
		ms_free(barmsg);
	}
	
	call->rtsp_endpoint_name = ms_strdup(name);

	MSList *rtsp_sub_list = ms_rtsp_connect(url, call->video_port, call->audio_port, dest_addr, &call->remoteRtspClient, &call->remoteRtspMediaSession);
	
	sprintf(to_uri, "sip:%s", dest_addr);
	LinphoneAddress *from=linphone_core_get_primary_contact_parsed(lc);
	LinphoneAddress *to = linphone_address_new(to_uri);
	call->log=linphone_call_log_new(call, from, to);
	call->owns_call_log=TRUE;

	if(rtsp_sub_list == NULL){
		ms_warning("we had a problem connect to rtsp server");
	...
	lc->current_rtsp_call = call;
	linphone_call_set_state (call,LinphoneCallOutgoingInit,"Starting outgoing call");
	linphone_call_init_media_streams(call);


	unsigned int id=rand() & 0xfff;
	SalMediaDescription *new_md = sal_media_description_new();
	new_md->session_id=id;
	new_md->session_ver=id;
	new_md->nstreams=1;
	strncpy(new_md->addr,dest_addr,sizeof(new_md->addr));
	strncpy(new_md->username,"temp",sizeof(new_md->username));
	new_md->bandwidth=linphone_core_get_download_bandwidth(lc);

	linphone_call_set_state(call,LinphoneCallOutgoingProgress,"Outgoing call in progress");


	MSList *it;
	for(it=rtsp_sub_list;it!=NULL;it=ms_list_next(it)){
		MSRtspSubSession *rtsp_subsession = (MSRtspSubSession *)it->data;
		if(strcmp(rtsp_subsession->mime_type, "H264") == 0) {
				if (call->params.has_video){
					new_md->nstreams++;
					strncpy(new_md->streams[1].addr,rtsp_subsession->addr,sizeof(new_md->streams[1].addr));
					new_md->streams[1].port=rtsp_subsession->server_port == 0 ? 65500 : rtsp_subsession->server_port;
					new_md->streams[1].proto=SalProtoRtpAvp;
					new_md->streams[1].type=SalVideo;
					new_md->streams[1].dir = SalStreamRecvOnly;
					MSList *l=NULL;
					PayloadType *pt = payload_type_new();
					pt->type=PAYLOAD_VIDEO;
					payload_type_set_number(pt,rtsp_subsession->payload_type);
					payload_type_set_flag(pt,PAYLOAD_TYPE_ENABLED);
					payload_type_set_flag(pt,PAYLOAD_TYPE_FLAG_CAN_SEND);
					payload_type_set_flag(pt,PAYLOAD_TYPE_FLAG_CAN_RECV);
					pt->mime_type = ms_strdup(rtsp_subsession->mime_type);
					pt->clock_rate = rtsp_subsession->clock_rate;
					payload_type_set_recv_fmtp(pt, rtsp_subsession->recv_fmtp);
					l=ms_list_append(l,pt);
					new_md->streams[1].payloads=l;
				}
		}
		if(strcmp(rtsp_subsession->mime_type, "PCMA") == 0 || strcmp(rtsp_subsession->mime_type, "PCMU") == 0) {
            strncpy(new_md->streams[0].addr, rtsp_subsession->addr,sizeof(new_md->streams[0].addr));
			new_md->streams[0].port=rtsp_subsession->server_port == 0 ? 65501 : rtsp_subsession->server_port;
			new_md->streams[0].proto=SalProtoRtpAvp;
			new_md->streams[0].type=SalAudio;
			new_md->streams[0].dir = SalStreamRecvOnly;
			MSList *l=NULL;
			PayloadType *pt = payload_type_new();
	...
	if (lc->vtable.display_status!=NULL) {
		char *msg=ms_strdup_printf(_("Call answered by %s."),name);
	...
	ms_rtsp_play(call->remoteRtspClient, call->remoteRtspMediaSession);

	linphone_call_set_state(call,LinphoneCallStreamsRunning,"Connected (streams running)");
    	//env->taskScheduler().doEventLoop(); // does not return
	linphone_call_params_destroy(p);
	ms_list_free(rtsp_sub_list);
	 
	return call;
}

// added for rtsp by liuhong end
...
int linphone_core_terminate_call(LinphoneCore *lc, LinphoneCall *the_call)
{
	...
	sal_call_terminate(call->op);
	// modify by liuhong for rtsp call start
	if(linphone_call_is_rtsp_call(call)) {
		linphone_core_rtsp_teardown_for_call(call);
	}
	// modify by liuhong for rtsp call end
	terminate_call(lc,call);
	if(call->conf_uid)
		ms_free(call->conf_uid);
...
{
	return lc->current_rtsp_call;
}
// add by liuhong for rtsp end
...
void linphone_core_set_playback_gain_db (LinphoneCore *lc, float gaindb){
	...
	lc->sound_conf.soft_play_lev=(char)gaindb;
...
LinphoneFirewallPolicy linphone_core_get_firewall_policy(const LinphoneCore *lc){
	return (LinphoneFirewallPolicy)lc->net_conf.firewall_policy;
}
...
/*
static RtpProfile *make_profile(const SalMediaDescription *md, const SalStreamDescription *desc, int *used_pt){
	int bw;
	const MSList *elem;
...
//	int remote_bw=0;
	...
	*used_pt=-1;
	...
	for(elem=desc->payloads;elem!=NULL;elem=elem->next){
		...
		int number;
		ms_warning("A payload type with number %d %d already exists in profile !", pt->flags, pt->flags & PAYLOAD_TYPE_FLAG_CAN_SEND);
...
void toggle_video_preview(LinphoneCore *lc, bool_t val){
#ifdef VIDEO_ENABLED
	if (val){
		if (lc->previewstream==NULL){
			lc->previewstream=video_preview_new();
			// modify by liuhong for rtsp start
			if(strcasecmp(lc->video_conf.device->desc->driver_type, "RTSP") != 0) {
...
//added by liuhong for rtsp
void linphone_core_set_cam_name(LinphoneCore *lc, const char *id, const char *name) {
	MSWebCam *cam = ms_web_cam_manager_get_cam(ms_web_cam_manager_get(),id);
	if(cam != NULL) {
		ms_free(cam->name);
		ms_free(cam->id);
		cam->name = ms_strdup(name);
		cam->id = ms_strdup_printf("%s: %s",cam->desc->driver_type,cam->name);
		lc->video_conf.rtsp_preview_url = ms_strdup(name);// could be memory leak
 ...
 * @ingroup media_parameters
**/
unsigned long linphone_core_get_native_rtsp_video_window_id(const LinphoneCore *lc){
#ifdef VIDEO_ENABLED
	LinphoneCall *call=linphone_core_get_current_rtsp_call(lc);
	if (call && call->videostream)
...
**/
void linphone_core_set_native_rtsp_video_window_id(LinphoneCore *lc, unsigned long id){
#ifdef VIDEO_ENABLED
	LinphoneCall *call=linphone_core_get_current_rtsp_call(lc);
...
#endif
	{	{MS_VIDEO_SIZE_WVGA_W,MS_VIDEO_SIZE_WVGA_H}	,	"wvga"	},
...
void rtp_config_uninit(LinphoneCore *lc)
{
	...
	lp_config_set_int(lc->config,"rtp","rtsp_audio_rtp_port",config->audio_rtsp_rtp_port);
	lp_config_set_int(lc->config,"rtp","rtsp_video_rtp_port",config->video_rtsp_rtp_port);
}
...
{
	lp_config_set_string(lc->config,"video","rtsp_preview_url",lc->video_conf.rtsp_preview_url);//add by liuhong for rtsp
...
static void linphone_core_uninit(LinphoneCore *lc)
{
	...
	while(lc->calls)
	{
		LinphoneCall *the_call = (LinphoneCall*)lc->calls->data;
...
bool_t linphone_core_set_admin(LinphoneCore *lc,bool_t is_admin){
	lc->is_admin = is_admin;
	return lc->is_admin;
}
