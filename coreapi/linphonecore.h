...
bool_t linphone_call_is_rtsp_call(LinphoneCall *call);// added by liuhong for rtsp
...
MS2_PUBLIC float linphone_call_get_play_volume(LinphoneCall *call);
MS2_PUBLIC float linphone_call_get_record_volume(LinphoneCall *call);
...
**/
MS2_PUBLIC typedef struct _LinphoneProxyConfig LinphoneProxyConfig;
...
MS2_PUBLIC bool_t linphone_core_set_admin(LinphoneCore *lc,bool_t is_admin);
MS2_PUBLIC LinphoneProxyConfig *linphone_proxy_config_new(void);
MS2_PUBLIC int linphone_proxy_config_set_server_addr(LinphoneProxyConfig *obj, const char *server_addr);
MS2_PUBLIC int linphone_proxy_config_set_identity(LinphoneProxyConfig *obj, const char *identity);
...
MS2_PUBLIC void linphone_core_enable_logs(FILE *file);
MS2_PUBLIC void linphone_core_enable_logs_with_cb(OrtpLogFunc logfunc);
MS2_PUBLIC void linphone_core_disable_logs(void);
/*sets the user-agent string in sip messages, must be set before linphone_core_new() or linphone_core_init() */
MS2_PUBLIC void linphone_core_set_user_agent(const char *ua_name, const char *version);
MS2_PUBLIC const char *linphone_core_get_version(void);

MS2_PUBLIC LinphoneCore *linphone_core_new(const LinphoneCoreVTable *vtable,
						const char *config_path, const char *factory_config, void* userdata);

/* function to be periodically called in a main loop */
MS2_PUBLIC void linphone_core_iterate(LinphoneCore *lc);

MS2_PUBLIC LinphoneAddress * linphone_core_interpret_url(LinphoneCore *lc, const char *url);

MS2_PUBLIC LinphoneCall * linphone_core_invite(LinphoneCore *lc, const char *url);
...
MS2_PUBLIC void linphone_core_rtsp_teardown_for_call(LinphoneCall *call);
// add for rtsp by liuhong end

MS2_PUBLIC LinphoneCall * linphone_core_invite_address(LinphoneCore *lc, const LinphoneAddress *addr);

MS2_PUBLIC LinphoneCall * linphone_core_invite_with_params(LinphoneCore *lc, const char *url, const LinphoneCallParams *params);

MS2_PUBLIC LinphoneCall * linphone_core_invite_address_with_params(LinphoneCore *lc, const LinphoneAddress *addr, const LinphoneCallParams *params);
...
MS2_PUBLIC LinphoneCall *linphone_core_get_current_call(const LinphoneCore *lc);

LinphoneCall *linphone_core_get_current_rtsp_call(const LinphoneCore *lc);

MS2_PUBLIC int linphone_core_accept_call(LinphoneCore *lc, LinphoneCall *call);

MS2_PUBLIC int linphone_core_terminate_call(LinphoneCore *lc, LinphoneCall *call);
...
MS2_PUBLIC void linphone_core_set_download_bandwidth(LinphoneCore *lc, int bw);
MS2_PUBLIC void linphone_core_set_upload_bandwidth(LinphoneCore *lc, int bw);

MS2_PUBLIC int linphone_core_get_download_bandwidth(const LinphoneCore *lc);
MS2_PUBLIC int linphone_core_get_upload_bandwidth(const LinphoneCore *lc);

MS2_PUBLIC void linphone_core_enable_adaptive_rate_control(LinphoneCore *lc, bool_t enabled);
MS2_PUBLIC bool_t linphone_core_adaptive_rate_control_enabled(const LinphoneCore *lc);
/**
 ...
 * @ingroup media_parameters
 */
MS2_PUBLIC int  linphone_core_get_download_ptime(LinphoneCore *lc);

MS2_PUBLIC void linphone_core_set_upload_ptime(LinphoneCore *lc, int ptime);

MS2_PUBLIC int linphone_core_get_upload_ptime(LinphoneCore *lc);

/* returns a MSList of PayloadType */
MS2_PUBLIC const MSList *linphone_core_get_audio_codecs(const LinphoneCore *lc);

MS2_PUBLIC int linphone_core_set_audio_codecs(LinphoneCore *lc, MSList *codecs);
/* returns a MSList of PayloadType */
MS2_PUBLIC const MSList *linphone_core_get_video_codecs(const LinphoneCore *lc);

MS2_PUBLIC int linphone_core_set_video_codecs(LinphoneCore *lc, MSList *codecs);

MS2_PUBLIC bool_t linphone_core_payload_type_enabled(LinphoneCore *lc, PayloadType *pt);

MS2_PUBLIC int linphone_core_enable_payload_type(LinphoneCore *lc, PayloadType *pt, bool_t enable);

MS2_PUBLIC PayloadType* linphone_core_find_payload_type(LinphoneCore* lc, const char* type, int rate) ;

MS2_PUBLIC const char *linphone_core_get_payload_type_description(LinphoneCore *lc, PayloadType *pt);

MS2_PUBLIC bool_t linphone_core_check_payload_type_usability(LinphoneCore *lc, PayloadType *pt);

MS2_PUBLIC int linphone_core_add_proxy_config(LinphoneCore *lc, LinphoneProxyConfig *config);

MS2_PUBLIC void linphone_core_clear_proxy_config(LinphoneCore *lc);

MS2_PUBLIC void linphone_core_remove_proxy_config(LinphoneCore *lc, LinphoneProxyConfig *config);

MS2_PUBLIC const MSList *linphone_core_get_proxy_config_list(const LinphoneCore *lc);

MS2_PUBLIC void linphone_core_set_default_proxy(LinphoneCore *lc, LinphoneProxyConfig *config);

MS2_PUBLIC void linphone_core_set_default_proxy_index(LinphoneCore *lc, int index);
...
MS2_PUBLIC int linphone_core_get_audio_rtsp_port(const LinphoneCore *lc);

MS2_PUBLIC int linphone_core_get_video_rtsp_port(const LinphoneCore *lc);
// added by liuhong for rtsp end

MS2_PUBLIC int linphone_core_get_audio_port(const LinphoneCore *lc);

MS2_PUBLIC int linphone_core_get_video_port(const LinphoneCore *lc);

MS2_PUBLIC int linphone_core_get_nortp_timeout(const LinphoneCore *lc);

//added by liuhong for rtsp start
MS2_PUBLIC void linphone_core_set_audio_rtsp_port(LinphoneCore *lc, int port);

MS2_PUBLIC void linphone_core_set_video_rtsp_port(LinphoneCore *lc, int port);

MS2_PUBLIC const char * linphone_core_get_rtsp_preview_url(LinphoneCore *lc);
//added by liuhong for rtsp end

MS2_PUBLIC void linphone_core_set_audio_port(LinphoneCore *lc, int port);

MS2_PUBLIC void linphone_core_set_video_port(LinphoneCore *lc, int port);

MS2_PUBLIC void linphone_core_set_nortp_timeout(LinphoneCore *lc, int port);
...
MS2_PUBLIC void linphone_core_set_sip_port(LinphoneCore *lc, int port);

MS2_PUBLIC int linphone_core_get_sip_port(LinphoneCore *lc);

MS2_PUBLIC int linphone_core_set_sip_transports(LinphoneCore *lc, const LCSipTransports *transports);

MS2_PUBLIC int linphone_core_get_sip_transports(LinphoneCore *lc, LCSipTransports *transports);
/**
 *
...
MS2_PUBLIC void linphone_core_set_stun_server(LinphoneCore *lc, const char *server);

MS2_PUBLIC const char * linphone_core_get_stun_server(const LinphoneCore *lc);

MS2_PUBLIC void linphone_core_set_nat_address(LinphoneCore *lc, const char *addr);

MS2_PUBLIC const char *linphone_core_get_nat_address(const LinphoneCore *lc);

MS2_PUBLIC void linphone_core_set_firewall_policy(LinphoneCore *lc, LinphoneFirewallPolicy pol);

MS2_PUBLIC LinphoneFirewallPolicy linphone_core_get_firewall_policy(const LinphoneCore *lc);

MS2_PUBLIC const char * linphone_core_get_relay_addr(const LinphoneCore *lc);

MS2_PUBLIC int linphone_core_set_relay_addr(LinphoneCore *lc, const char *addr);

/* sound functions */
/* returns a null terminated static array of string describing the sound devices */
MS2_PUBLIC const char**  linphone_core_get_sound_devices(LinphoneCore *lc);
MS2_PUBLIC bool_t linphone_core_sound_device_can_capture(LinphoneCore *lc, const char *device);
MS2_PUBLIC bool_t linphone_core_sound_device_can_playback(LinphoneCore *lc, const char *device);
MS2_PUBLIC int linphone_core_get_ring_level(LinphoneCore *lc);
MS2_PUBLIC int linphone_core_get_play_level(LinphoneCore *lc);
MS2_PUBLIC int linphone_core_get_rec_level(LinphoneCore *lc);
MS2_PUBLIC void linphone_core_set_ring_level(LinphoneCore *lc, int level);
MS2_PUBLIC void linphone_core_set_play_level(LinphoneCore *lc, int level);

MS2_PUBLIC void linphone_core_set_playback_gain_db(LinphoneCore *lc, float level);

MS2_PUBLIC float linphone_core_get_playback_gain_db(LinphoneCore *lc);
MS2_PUBLIC void linphone_core_set_rec_level(LinphoneCore *lc, int level);
MS2_PUBLIC const char * linphone_core_get_ringer_device(LinphoneCore *lc);
MS2_PUBLIC const char * linphone_core_get_playback_device(LinphoneCore *lc);
MS2_PUBLIC const char * linphone_core_get_capture_device(LinphoneCore *lc);
MS2_PUBLIC int linphone_core_set_ringer_device(LinphoneCore *lc, const char * devid);
MS2_PUBLIC int linphone_core_set_playback_device(LinphoneCore *lc, const char * devid);
MS2_PUBLIC int linphone_core_set_capture_device(LinphoneCore *lc, const char * devid);
MS2_PUBLIC char linphone_core_get_sound_source(LinphoneCore *lc);
MS2_PUBLIC void linphone_core_set_sound_source(LinphoneCore *lc, char source);
MS2_PUBLIC void linphone_core_set_ring(LinphoneCore *lc, const char *path);
MS2_PUBLIC const char *linphone_core_get_ring(const LinphoneCore *lc);
MS2_PUBLIC void linphone_core_verify_server_certificates(LinphoneCore *lc, bool_t yesno);
MS2_PUBLIC void linphone_core_set_root_ca(LinphoneCore *lc, const char *path);
MS2_PUBLIC void linphone_core_set_ringback(LinphoneCore *lc, const char *path);
MS2_PUBLIC const char * linphone_core_get_ringback(const LinphoneCore *lc);

MS2_PUBLIC void linphone_core_set_remote_ringback_tone(LinphoneCore *lc,const char *);
MS2_PUBLIC const char *linphone_core_get_remote_ringback_tone(const LinphoneCore *lc);

MS2_PUBLIC int linphone_core_preview_ring(LinphoneCore *lc, const char *ring,LinphoneCoreCbFunc func,void * userdata);
MS2_PUBLIC void linphone_core_enable_echo_cancellation(LinphoneCore *lc, bool_t val);
MS2_PUBLIC bool_t linphone_core_echo_cancellation_enabled(LinphoneCore *lc);

MS2_PUBLIC void linphone_core_enable_echo_limiter(LinphoneCore *lc, bool_t val);
MS2_PUBLIC bool_t linphone_core_echo_limiter_enabled(const LinphoneCore *lc);

MS2_PUBLIC void linphone_core_enable_agc(LinphoneCore *lc, bool_t val);
MS2_PUBLIC bool_t linphone_core_agc_enabled(const LinphoneCore *lc);

MS2_PUBLIC void linphone_core_mute_mic(LinphoneCore *lc, bool_t muted);
/**
 * return mic state.
 *
 * @ingroup media_parameters
**/
MS2_PUBLIC bool_t linphone_core_is_mic_muted(LinphoneCore *lc);

MS2_PUBLIC bool_t linphone_core_is_audio_muted(LinphoneCore *lc);
MS2_PUBLIC bool_t linphone_core_is_rtp_muted(LinphoneCore *lc);

MS2_PUBLIC bool_t linphone_core_get_rtp_no_xmit_on_audio_mute(const LinphoneCore *lc);
MS2_PUBLIC void linphone_core_set_rtp_no_xmit_on_audio_mute(LinphoneCore *lc, bool_t val);
...
/* video support */
MS2_PUBLIC void linphone_core_enable_video(LinphoneCore *lc, bool_t vcap_enabled, bool_t display_enabled);
MS2_PUBLIC bool_t linphone_core_video_enabled(LinphoneCore *lc);
...
MS2_PUBLIC const MSVideoSizeDef *linphone_core_get_supported_video_sizes(LinphoneCore *lc);
MS2_PUBLIC void linphone_core_set_preferred_video_size(LinphoneCore *lc, MSVideoSize vsize);
MS2_PUBLIC MSVideoSize linphone_core_get_preferred_video_size(LinphoneCore *lc);
MS2_PUBLIC void linphone_core_set_preferred_video_size_by_name(LinphoneCore *lc, const char *name);

MS2_PUBLIC void linphone_core_enable_video_preview(LinphoneCore *lc, bool_t val);
MS2_PUBLIC bool_t linphone_core_video_preview_enabled(const LinphoneCore *lc);
...
MS2_PUBLIC const char**  linphone_core_get_video_devices(const LinphoneCore *lc);
MS2_PUBLIC int linphone_core_set_video_device(LinphoneCore *lc, const char *id);
MS2_PUBLIC const char *linphone_core_get_video_device(const LinphoneCore *lc);

/* Set static picture to be used when "Static picture" is the video device */
MS2_PUBLIC int linphone_core_set_static_picture(LinphoneCore *lc, const char *path);

/* Set and get frame rate for static picture */
MS2_PUBLIC int linphone_core_set_static_picture_fps(LinphoneCore *lc, float fps);
MS2_PUBLIC float linphone_core_get_static_picture_fps(LinphoneCore *lc);

// added by liuhong for rtsp start
MS2_PUBLIC unsigned long linphone_core_get_native_rtsp_video_window_id(const LinphoneCore *lc);
MS2_PUBLIC void linphone_core_set_native_rtsp_video_window_id(LinphoneCore *lc, unsigned long id);
// added by liuhong for rtsp end

/*function to be used for eventually setting window decorations (icons, title...)*/
MS2_PUBLIC unsigned long linphone_core_get_native_video_window_id(const LinphoneCore *lc);
MS2_PUBLIC void linphone_core_set_native_video_window_id(LinphoneCore *lc, unsigned long id);

MS2_PUBLIC unsigned long linphone_core_get_native_preview_window_id(const LinphoneCore *lc);
MS2_PUBLIC void linphone_core_set_native_preview_window_id(LinphoneCore *lc, unsigned long id);

MS2_PUBLIC void linphone_core_use_preview_window(LinphoneCore *lc, bool_t yesno);
MS2_PUBLIC int linphone_core_get_device_rotation(LinphoneCore *lc );
void linphone_core_set_device_rotation(LinphoneCore *lc, int rotation); 
/*play/record support: use files instead of soundcard*/
MS2_PUBLIC void linphone_core_use_files(LinphoneCore *lc, bool_t yesno);
MS2_PUBLIC void linphone_core_set_play_file(LinphoneCore *lc, const char *file);
MS2_PUBLIC void linphone_core_set_record_file(LinphoneCore *lc, const char *file);

MS2_PUBLIC void linphone_core_play_dtmf(LinphoneCore *lc, char dtmf, int duration_ms);
MS2_PUBLIC void linphone_core_stop_dtmf(LinphoneCore *lc);

MS2_PUBLIC int linphone_core_get_current_call_duration(const LinphoneCore *lc);


MS2_PUBLIC int linphone_core_get_mtu(const LinphoneCore *lc);
MS2_PUBLIC void linphone_core_set_mtu(LinphoneCore *lc, int mtu);
...
MS2_PUBLIC struct _LpConfig *linphone_core_get_config(LinphoneCore *lc);
...
MS2_PUBLIC void linphone_core_destroy(LinphoneCore *lc);
...
MS2_PUBLIC void linphone_core_set_zrtp_secrets_file(LinphoneCore *lc, const char* file);
...
bool_t linphone_core_is_admin(LinphoneCore *lc);
