...
int sal_listen_port(Sal *ctx, const char *addr, int port, SalTransport tr, int is_secure){
	...
	use_rports = ctx->use_rports; // Copy char to int to avoid bad alignment
	eXosip_set_option(EXOSIP_OPT_USE_RPORT,&use_rports);
	dont_use_101 = !ctx->use_101; // Copy char to int to avoid bad alignment
...
static void set_username(SalOp *op, osip_message_t *req){
	...
	osip_message_header_get_byname(req,"username",0,&h);
	if (h){
		if (h->hvalue && h->hvalue[0]!='\0'){
			sal_op_set_username(op, ms_strdup(h->hvalue));
		}
	}
}
static void set_userid(SalOp *op, osip_message_t *req){
	osip_header_t *h=NULL;
	...
	osip_message_header_get_byname(req,"userid",0,&h);
	if (h){
		if (h->hvalue && h->hvalue[0]!='\0'){
			sal_op_set_userid(op, ms_strdup(h->hvalue));
		}
	}
}
...
static void inc_new_call(Sal *sal, eXosip_event_t *ev){
	...
	char *from1 =NULL;
	osip_from_to_str(ev->request->from,&from1);
	ms_message("request port is %s ", from1);
...
// added by liuhong for desktop share
void sal_op_set_desktop_share(SalOp *op, bool_t is_desktop_share) {
...
static void call_message_new(Sal *sal, eXosip_event_t *ev){
	...
	if (ev->request){
		if (MSG_IS_INFO(ev->request)){
			...
			osip_message_header_get_byname(ev->request,"Confuid",0,&uid);
			call->conf_uid = ms_strdup(uid->hvalue);
