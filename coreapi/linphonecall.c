...
static void linphone_call_init_common(LinphoneCall *call, LinphoneAddress *from, LinphoneAddress *to){
	...
	call->remoteRtspMediaSession = NULL;// added for rtsp call by liuhong
	call->remoteRtspClient = NULL;// added for rtsp call by liuhong
	call->conf_uid = NULL;
}
...
bool_t linphone_call_is_rtsp_call(LinphoneCall *call) {
	if(call->remoteRtspClient != NULL) {
		return TRUE;
	} else {
		return FALSE;
	}
...
char *linphone_call_get_conf_uid(LinphoneCall *call) {
	return call->conf_uid;
}
...
static void linphone_call_start_video_stream(LinphoneCall *call, const char *cname,bool_t all_inputs_muted){
	...
	if (vstream!=NULL && vstream->dir!=SalStreamInactive && vstream->port!=0) {
		...
		ms_message("vstream is %d,%d %d", vstream->dir, vstream->port, used_pt);
		if (used_pt!=-1){
			...
			if (!is_inactive){
				...
				if(call->params.is_desktop_share) {
					call->videostream->dir = VideoStreamSendOnly;//added by liuhong for desktop share
#if defined(__MINGW32__) || defined (HAVE_DIRECTSHOW)
					extern MSWebCamDesc ms_push_source_cam_desc;//added by liuhong for desktop push
...
void linphone_call_background_tasks(LinphoneCall *call, bool_t one_second_elapsed){
	...
	if (call->state==LinphoneCallStreamsRunning && one_second_elapsed){
		...
		video_load = audio_load;
		audio_load = video_load;
