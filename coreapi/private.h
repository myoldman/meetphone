...
struct _LinphoneCall
{
	...
	void *remoteRtspMediaSession;
	void *remoteRtspClient;
	char *rtsp_endpoint_name;
	char *conf_uid;
};
...
MS2_PUBLIC void linphone_proxy_config_update(LinphoneProxyConfig *cfg);
...
typedef struct rtp_config
{
	int audio_rtp_port;
	int video_rtp_port;
	int audio_rtsp_rtp_port;
	int video_rtsp_rtp_port;
...
typedef struct video_config{
	...
	const char **cams;
	MSVideoSize vsize;
	const char *rtsp_preview_url;//add for rtsp by liuhong
	bool_t capture;
	bool_t show_local;
	bool_t display;
...
struct _LinphoneCore
{
	...
	LinphoneCall *current_rtsp_call; /* the current rtsp call */
	...
	bool_t is_admin;// add for rtsp call by liuhong
};
